...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec2 cuv = fragCoord.xy / iResolution.xy;
    vec2 aspect = vec2(1., iResolution.y/iResolution.x);
    vec2 uv = 0.5 + (cuv - 0.5)*aspect;
    ...
    vec4 bg = texture(iChannel0, cuv);
    
    // Blend heart pattern (d) over the background
    float intensity = clamp(max(d.x, max(d.y, d.z)), 0.0, 1.0);
    fragColor = mix(bg, vec4(d, 1.0), intensity);
}
