...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  // ── GHOSTTY FIX ────────────────────────────────────────────────────────────
  // iChannel0 = rendered terminal frame (alpha encodes text vs background).
  // Dim the raymarched scene, then composite terminal text on top.
  // Adjust the 0.25 multiplier to taste: 0.1 = very dark, 0.5 = bright.
  vec4 term = texture(iChannel0, uv);
  col = mix(col * 0.25, term.rgb, term.a);
  // ───────────────────────────────────────────────────────────────────────────
